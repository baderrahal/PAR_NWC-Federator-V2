# CLAUDE.md as it stood on 2026-09-12, before F38

Measurement history, not current. This is the whole file as it was when the rules in it
were split into CLAUDE.md, which holds the rules for every file, and .claude/rules, which
holds the rules per folder. It is kept whole because every rule in it was read off a run
or a DLL on the date it names, and those dates and numbers are the reasons behind the
rules. Where this file and the rules files disagree, the rules files are current.
