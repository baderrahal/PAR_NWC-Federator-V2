...
79. From PART 3 of the scale round on 2026-09-21. The workbook block count line said `THE WORKBOOK MUST CARRY A BLOCK FOR EVERY TEST IN THE FILE` and nothing anywhere enforced it. `GroupFacts` carries no report check result of any kind, so `GroupJudgement` cannot see it, and `1A0415` reported DONE over a line printed in capitals. The scale round took the word MUST out rather than making it a gate, and the reason is that the counter ITSELF was wrong at the time: it read 0 blocks against 1830 on a workbook that carried all 1830, so a gate would have failed a group whose output was correct. That is the same fault in the other direction as the run that once reported a clean 22 group run as FAILED, which is why this repo has the rule that a report check never fails a group. The count is right now and the finding is counted into RESULT. Should a workbook genuinely missing a block now FAIL its group, or stay a finding a person acts on

    Answer:

80. From PART 4 and 6a of the scale round on 2026-09-21, and it is the round's main measurement. Writing one saved viewpoint costs time in proportion to HOW MANY THE DOCUMENT ALREADY HOLDS. Measured with a control on one open copy of 1A02MM, same models, same 2,606 items, same 1,830 tests, one thing changed at a time: with 568 saved viewpoints in it, 40 writes cost 1,878 ms each, clearing every clash result changed nothing at 1,873 ms, and `SavedViewpoints.Clear()` collapsed it to 6.5 ms. The read backs are 5 per cent of the step and the dimming is 0.14 per cent, so neither is the thing to cut, and the COM folder route 5p measured is already the default. Within a run the climb is mild, 7 ms at 25 written to 14 ms at 400, so it is the viewpoints read OFF THE NWF that cost. `ViewpointSettings.MaxPerGroup` is a ceiling that postpones the problem and does not solve it, and the thing a ceiling costs is the viewpoints a person does not get. THREE ROUTES ARE VISIBLE AND NONE IS MEASURED: write the viewpoints into a document that does not hold the old ones and merge after, keep the old ones out of the .NET tree while writing so the read back is the only thing that materialises them, or write them to a separate NWF beside the federation. Which route, and what does it cost to measure each
