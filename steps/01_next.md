...
- DONE on 2026-09-21. `tools/checks/check-facts.sh` with `tools/checks/facts.tsv`. THE VALUE IS READ OFF THE CODE AND NEVER OFF THE REGISTRY, or the registry would be one more copy to go stale like the four that already had. Proved both ways: over the real files it reads 13 lines across 5 facts and passes, and over `tools/checks/broken/facts-wrong.md` it names all five faults by file and line and exits 1. Both steps are in Actions
- IT PASSED VACUOUSLY ON THREE OF ITS OWN FIVE FACTS FIRST, which is the fault it exists to catch. The first registry's subject patterns matched no line at all for the threshold, the tick boxes and the .tsv, so those three reported a pass having read nothing. It counts what it examined now, says the count per fact, and a named fact no prose line claims is a FAULT
- AND ONE OF THE FOUR NAMED FACTS CANNOT BE CHECKED AT ALL, said here rather than quietly dropped. THE .tsv EVENT KINDS ARE NOT A FACT THE CODE HOLDS. They are string literals at their call sites, `"test passed"` in ClashRunner, `"step finished"` in RunLog, `"census after"` beside it, with NO list anywhere. That is exactly why that one drifted and why nobody could keep it right, and no check can read a number that is not written down once. The .tsv fact checked instead is the COLUMN COUNT, which `EventRow.FieldCount` does hold in one place. Making the kinds checkable means first giving them a home, which is a build and not a check
- AND THE TICK BOX READER ANSWERED THE WRONG QUESTION FIRST. Counting `CheckBox` in the XAML gives 22. The fact the prose states, at `steps/03_bader_next.md` step 212, is how many are VISIBLE WITHOUT OPENING ANYTHING, which is four, because the rest sit inside an expander that starts closed

## F97 The rebuild that was thrown away every run

- Closes the twenty eight, which was read as stability for two rounds
- Files `src/Federator.Core/Sets/SetBuildOutcome.cs`, `src/Federator.Core/Sets/SetDrift.cs`, `src/Federator.Addin/Engine/FederationEngine.cs`
- DONE on 2026-09-21. `BuildTheSetsFromTheFile` returned `PutAnythingIn`, which is `CreatedCount > 0`, and a set already at its path is rebuilt IN PLACE and creates nothing. So a group whose only change was a rebuild reported that it had changed nothing, the NWF was never saved, and the same fourteen sets drifted again next run, forever. `RebuiltCount` existed the whole time and nothing read it
- THE EVIDENCE IS THE TWO GROUPS. The drift appears in exactly `1000BS` and `1A02MS`, which are the only two whose NWF comes back the size it went in, 13,091 and 81,957 bytes. The eight that write their NWF for some other reason never drift twice. `docs/history/scan.md` 6b
- AND EVERY ONE OF THE 28 IS A CASE DIFFERENCE IN A WORKSET VALUE, `ME-DUCTWORK` against `ME-Ductwork` and three more. The drift line says WHY now, because two sentences differing in one letter's case are what three people have already had to compare character by character

## F98 A check that cannot fail is a bug, and this one could not

- Closes the `1A0415` finding: 0 blocks reported against 1830, over a workbook carrying all 1830
- Files `src/Federator.Core/Report/WorkbookCheck.cs`, `src/Federator.Core/Clash/CreationPlan.cs`, `src/Federator.Core/Diagnostics/RunLog.cs`
- DONE on 2026-09-21. The counter found a block by the literal `Clash Name` in column 3, and since Q73 an empty test is ONE ROW carrying its TOLERANCE in that same column 3 with no headings row, because `ColumnTestHeader` and `ColumnClashName` are both 3. A block is found by the test name in column A now, which is the one thing both shapes carry
- PROVED ON HIS OWN FILES AND NOT A FIXTURE. The fixed check reads `1A0415` at 1830 blocks and `1A04EP` at 1830 blocks, 109 full, 2566 clash rows, which is exactly that group's clash count
- AND THE ZERO WAS SWITCHING OFF EVERY OTHER CHECK. The cell, width, order, title and priority comparisons were gated on `Blocks == 1`, so none ran, and the block still said every column, fill, border, height and width matched. Nothing compared and everything matching are opposite answers and it printed both at once
- THE MUST IS GONE FROM THE LINE and the finding is counted into RESULT instead. Whether it should become a gate is Q79

## F99 The ceiling on how many viewpoints one group writes

- Closes nothing. It postpones Q80, which is the real fix
- Files `src/Federator.Core/Views/ViewpointSettings.cs`, `src/Federator.Core/Views/ClashViewpointPlan.cs`, `src/Federator.Core/Report/ReportOptions.cs`, the window
- DONE on 2026-09-21, off by default. It is NOT only about minutes. `1A04EP` ended with models 5 to 0, sets 61 to 0, tests 666 to 0 and results 2,566 to 0 DURING the viewpoints step, its last 1,408 viewpoints written without a hidden state because there was no model left to hide, and the NWF save then refused an empty document
- THE COST WAS MEASURED WITH A CONTROL AND IT IS NONE OF THE THREE THINGS ANYONE EXPECTED. Not the read backs, 5 per cent. Not the dimming, 0.14 per cent. Not the COM folder route, already the default. It is how many saved viewpoints the document ALREADY HOLDS: 1,878 ms a write with 568 in it, 1,873 ms after every clash result was cleared, 6.5 ms after the viewpoints were. `docs/history/scan.md` 6a
- A CAP IS NEVER SILENT. The block says the ceiling, how many went over it, and says so even when it was set and NOT reached, because a block silent about a ceiling reads as a run that had none
